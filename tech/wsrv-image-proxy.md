...
Since the 2026-08-07 switch to a text-only [DeepSeek](/tech/deepseek-api.md) call, no image is sent to
any model, so the old base64-download workaround (and the robots.txt problem that forced it) is gone.
