...
| Symptom | Cause | Fix |
|---|---|---|
| Run is green but **0 picks and no email**, `truncated: true` on Parse response | `finish_reason: length` — thinking tokens consumed the whole `max_tokens`. Observed 2026-08-07 at `max_tokens: 4000` (4000/4000 reasoning, empty content). | Raise `max_tokens` in Build request (now **16000**) — see [deepseek-api](/tech/deepseek-api.md#thinking) |
