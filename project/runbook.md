...
| **AI - Groq**: **413** `Request too large … tokens per minute (TPM): Limit 8000` | `prompt + max_tokens` exceeded the free tier's per-minute window. Groq counts the *ceiling*, not the answer, so this fails before generating and **retries cannot help** | lower `max_tokens` in Build request (currently 3000, sized against a ~4600-token worst case) — the arithmetic is in [groq](/tech/groq.md#tpm). Usually means the candidate list or captions grew |
