...
The [project](/project/thats-a-first-digest.md) is a single [n8n](/tech/n8n.md) workflow: **one linear
chain**, 13 nodes. It had a fork after **Build request** for the hashtag-counts branch until 2026-08-20,
when that feature was removed ([decisions](/project/decisions.md#drop-hashtag-counts)).

## Node chain

```
Schedule (06:00) → Config → Get sent → Apify Instagram → Apify TikTok
   → Build request → AI - Groq → Parse response → Has picks? ──(true)──► Gmail send
                                                                            │
                                                                            ▼
                                                              Split picks ─► Insert row
```
...
| **Config** | Set | `recipient`, `lookbackDays=30`. (The `hashtags[]` entry was dropped with the counts feature — the scraper nodes carry their own tag lists.) |
...
This applies to both Apify nodes. See [decisions](/project/decisions.md) and [apify](/tech/apify.md).
